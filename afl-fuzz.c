...
//ljq
khash_t(m32) *km32_state_snapshot;   
klist_t(lms) *kl_messages_m1,*kl_messages_m2_m3;
u8  use_snapshot;
s32 snapshot_flag = -1;
s32 can_snapshot_flag = -1;
/* PID of the snapshot */
// s32 snapshot_pid;    
s32 criu_dump_monitor_pid;     
u8* program_name;
s32 fds,fde;
u8 recvbuf[5];
int criu_restore_cout = 0;
int fuzz_one_count = 0;
FILE* snapshot_logfile;
s32 snapshot_logfile_fd;
FILE* prefix_save_logfile;
s32 prefix_save_logfile_fd;

u32 messages_sent_m1 = 0;
u32 messages_sent_m23 = 0;
int socket_fd = -1;
char *response_buf_m1 = NULL;
int response_buf_size_m1 = 0; //the size of the whole response buffer
u32 *response_bytes_m1 = NULL; //an array keeping accumulated response buffer size

char *response_buf_m23 = NULL;
int response_buf_size_m23 = 0; //the size of the whole response buffer
u32 *response_bytes_m23 = NULL; //an array keeping accumulated response buffer size

u32 MAX_BUF_SIZE = 0;
// u32 MAX_BUF_SIZE = 99999999;
s32 MAX_MESSAGES_COUNTS = 5;
bool choose_last_same_state = TRUE;
u32 stage_max_factor = 1;
// u32 MAX_BUF_SIZE = 0;
u32 now_buf_size = 0; 
s32 MAX_STAGE_CUR = 16;

u8 trace_bits_m1[MAP_SIZE];
...
if(use_snapshot) km32_state_snapshot = kh_init(m32);
}
...
void what_err_ret_mean(int ret)
{
  ...
  switch (ret) {
  case -EBADE:
    fprintf(snapshot_logfile, "RPC has returned fail\n");
    break;
  case -ECONNREFUSED:
    fprintf(snapshot_logfile, "Unable to connect to CRIU\n");
    break;
  case -ECOMM:
    fprintf(snapshot_logfile, "Unable to send/recv msg to/from CRIU\n");
    break;
  case -EINVAL:
    fprintf(snapshot_logfile, "CRIU doesn't support this type of request."
           "You should probably update CRIU\n");
    break;
  case -EBADMSG:
    fprintf(snapshot_logfile, "Unexpected response from CRIU."
           "You should probably update CRIU\n");
    break;
  default:
    fprintf(snapshot_logfile, "Unknown error type code."
           "You should probably update CRIU\n");
  }
}
...
s32 self_criu_restore() {
  ...
  //start to restore
  // int try_count = 0;
  // int target_state_pid;
  // khint_t k = kh_get(m32, km32_state_snapshot, target_state_id);
  // if (k != kh_end(km32_state_snapshot)) {
  //   target_state_pid = kh_val(km32_state_snapshot, k);
  // }
  // else PFATAL("no snapshot_pid restored in hashmap");
  ...
  if (error_times != 0) fprintf(snapshot_logfile, "error_times:%d\n", error_times);
  if (error_times == 10) stop_soon = 7;
  return fork_pid;
...
int send_over_network_m1()
{
  ...
  for (it = kl_begin(kl_messages_m1); it != kl_end(kl_messages_m1); it = kl_next(it)) {
    ...
    if (n != kl_val(it)->msize) {
      fprintf(snapshot_logfile, "error:send_over_network_m1 send error\n"); 
      goto HANDLE_RESPONSES_M1;
      // PFATAL("send_over_network_m1 send error");
    }
    ...
    if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m1, &response_buf_size_m1)) {
      ...
      fprintf(snapshot_logfile, "error send_over_network_m1 recv error\n"); 
      goto HANDLE_RESPONSES_M1;
    }
...
HANDLE_RESPONSES_M1:
  ...
  u8* fn = alloc_printf("/proc/%d", child_pid);
  if (access(fn, F_OK) != 0) {
    ck_free(fn);
  } 
  else {
  ...
  int i;
  for(i = 0; i < messages_sent_m1; i++) {
    response_bytes[i] = response_bytes_m1[i];
  }
...
int send_over_network_m23(s32 criu_pid)
{
  int n;
  u8 likely_buggy = 0;
  //write the request messages
  ...
  response_buf_m23 = (char *) ck_realloc(response_buf_m23, response_buf_size_m1 * sizeof(char));
  memcpy(response_buf_m23, response_buf_m1, response_buf_size_m1);
...
HANDLE_RESPONSES:
  ...
  response_buf = (char *)ck_realloc(response_buf, response_buf_size_m23 * sizeof(char));
  memcpy(response_buf, response_buf_m23, response_buf_size_m23);
  ...
  if (likely_buggy && false_negative_reduction) {
    if (waitpid(criu_pid, &status,0) <= 0) PFATAL("waitpid() failed");
  }

  else {

    child_pid = criu_pid;    //let handle_timeout function to kill criu_pid
    if (terminate_child && (criu_pid > 0)) kill(criu_pid, SIGTERM);

    u8 *temp = alloc_printf("kill_process %d",criu_pid);

    FILE *fp;
    if((fp = popen(temp,"r")) == NULL){
      PFATAL("error execute kill_process");
    }
    int d = fileno(fp); 
    fcntl(d, F_SETFL, O_NONBLOCK); 
    ck_free(temp);
    pid_t retpid = waitpid(criu_pid, &status,0);
...
void self_criu_dump() { 
  ...
  closedir(dirptr);
  u8* rm_dir_name = alloc_printf("cd %s && rm *",dir_name);
  system(rm_dir_name);
  ck_free(rm_dir_name);
  ...
  char temp[256];
  sprintf(temp, "criu dump --tree %d --images-dir %s -v4 -o dump.log --shell-job", child_pid , dir_name);
  ...
  int ret;
  khint_t k;
  k = kh_put(m32,km32_state_snapshot,target_state_id,&ret);
  kh_value(km32_state_snapshot, k) = child_pid;
  ...
  //     khint_t k;
  //     k = kh_put(m32,km32_state_snapshot,target_state_id,&ret);
  //   // kh_put(m32,km32_state_snapshot,target_state_id,&ret);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    //if have snapshot,criu restore , do nothing
    if (use_snapshot && snapshot_flag == 1) {
  ...
  //ljq 
  if (use_snapshot && snapshot_flag != -1 && snapshot_flag != 2) {
    ...
    else if (snapshot_flag == 0){

    memset(trace_bits_m1, 0, MAP_SIZE);
    socket_fd = send_over_network_m1();
    if (socket_fd == -2) {
      stop_soon = 7;
      s32 res;
      int status = 0;
    ...
    else PFATAL("error snapshot_flag value");
 
    status = send_over_network_m23(criu_pid);
...
AFLNET_REGIONS_SELECTION:;
  ...
  u32 M2_start_region_ID = 0, M2_start_region_ID_PREV = 0 , M2_region_count = 0;
  ...
  if (state_aware_mode) {
    ...
    if(use_snapshot && choose_last_same_state) {
      ...
      for(i = 0; i < queue_cur->region_count ; i++) {
        ...
        for(i = 0; i < queue_cur->region_count; i++) {
          ...
          if (regionalStateCount > 0) {
            ...
            if (reachableStateID == target_state_id) M2_start_region_ID_PREV = i+1;
        ...
        if (M2_start_region_ID_PREV != 0) M2_start_region_ID = M2_start_region_ID_PREV;
...
abandon_entry:
  ...
  if(use_snapshot && (snapshot_flag == 0 || snapshot_flag == 1) ) {
      delete_kl_messages(kl_messages_m1);
      delete_kl_messages(kl_messages_m2_m3);
      close(socket_fd);
      socket_fd = -1;
...
static void setup_snapshot_dir() {
  ...
  prefix_save_logfile_fd = open(fn1, O_WRONLY | O_CREAT | O_TRUNC , 0600);
  ck_free(fn1);
...
static void save_program_name(u32 argc, char** argv) {

  u32 len = 1, i;
  char *temp = "./";

  for (i = 0; i < argc; i++){
    if(strncmp(temp, argv[i], 2) == 0){

      len += strlen(argv[i]);
      program_name = ck_alloc(len);
      memcpy(program_name, argv[i], len-1 );
      break;

    }
  }
  *(program_name + len - 1) = 0;
  if(strlen(program_name) == 0) PFATAL("Can not find program name");
  ACTF("program_name:%s",program_name);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:P:KEq:s:RFc:l:b")) > 0)
    ...
    switch (opt) {
      ...
      case 'b': /* local port to connect from */
        //This option is only used for targets that send responses to a specific port number
        //The Kamailio SIP server is an example
  ...
  if (state_aware_mode) {
    ...
    while (1) {
      ...
      while(!selected_seed || selected_seed->region_count == 0) {
        ...
        // ACTF("target_state_id:%d,state_ids_count:%d",target_state_id,state_ids_count);
        ...
        if(use_snapshot){
          snapshot_flag = check_state_snapshot();
          snapshot_flag = 0;
          // ACTF("*****snapshot_flag******:%d", snapshot_flag);
        }
      ...
      // if(fuzz_one_count == 2) stop_soon = 1;
