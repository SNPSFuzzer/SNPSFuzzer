		...
		struct libsoccr_sk_data *data, unsigned data_size)
{
	...
	if (data->state == TCP_SYN_SENT && tcp_repair_off(sk->fd)){
		logerr("Can't turn TCP_REPAIR off");
		return -1;
	}
	...
	if (data->state == TCP_SYN_SENT && tcp_repair_on(sk->fd)){
		logerr("Can't turn TCP_REPAIR on");
		return -1;
	}
...
// 	loge("src:%s:%u, dst:%s:%u", inet_ntoa(sk->dst_addr->v4.sin_addr),ntohs(sk->dst_addr->v4.sin_port), inet_ntoa(sk->src_addr->v4.sin_addr),ntohs(sk->src_addr->v4.sin_port));
